---
title: "RUV_FF_vs_Mal_all_groups_Salmon"
author: "Raquel Pinho"
...
dir.create(file.path("C:/Users/rpmello/Desktop/Data Analysis RNASeq/Projeto_Salmon_DESeq_EdgeR/Output", sprintf("FF_vs_Mal_all_groups_Salmon_R_with_outliers")), showWarnings = FALSE)
output_dir<-file.path("C:/Users/rpmello/Desktop/Data Analysis RNASeq/Projeto_Salmon_DESeq_EdgeR/Output", sprintf("FF_vs_Mal_all_groups_Salmon_R_with_outliers"))
dir.create(file.path(output_dir, "Figures"), showWarnings = FALSE)
print("Comparison between FF and Mal independently of milk all sections together.")
...
selectMetadata <- metadata[which(metadata$Ecoli == "No"), ]
selectMetadata<- selectMetadata[order(selectMetadata$Diet),]
files <- selectMetadata$list_of_files
labels<- c(paste0(selectMetadata$Diet[1:12], 1:12), paste0(selectMetadata$Diet[13:48],1:36))
...
batch_effects$Group<-selectMetadata$Group
batch_effects$Diet<-selectMetadata$Diet
batch_effects
```
...
Creating a DESeqDataSet for the in this case we will use the design ~Diet, because, for know we are only interested in the normalized counts to filter low counts and calculate the variation varible with RUV.  

```{r DESeq_norm}
library(DESeq2)
dds <-DESeqDataSetFromTximport(txidata, batch_effects, as.formula("~Diet"))
...
The number of sig DE genes with mod1 (~Diet) was:
...
set_n$Diet<-factor(set_n$Diet)
plotRLE(set_n, outline=F, col=colors[set_n$Diet])
plotPCA(set_n,col=colors[set_n$Diet])
```
...
Poisson distance rlog transformed data
...
library(pheatmap)
rlogdds<- rlog(dds)
saveRDS(rlogdds,file = file.path(output_dir,sprintf("FF_vs_Mall_all_groups_rlog_counts_Sal_with_outliers.RDS")))
poisd<-PoissonDistance(t(DESeq2::counts(dds)))
samplePoisDistMatrix<-as.matrix(poisd$dd)
row.names(samplePoisDistMatrix)<-paste(rlogdds$Diet, rlogdds$Group, sep="_")
## sample dist
...
```{r PCA_plot}
library(ggplot2)
z <-plotPCA(rlogdds, intgroup = c( "Diet"))
z+ geom_text(aes(label = selectMetadata$Pig))

# Using the both
percentVar <- round(100 * attr(z, "percentVar"))
ggplot(as.data.frame(z$data), aes(PC1, PC2, shape=Diet, color=rlogdds$Group)) +  geom_point(size=3) +
...
saveRDS(resSig_Salmon, file = file.path(output_dir,sprintf("FF_vs_Mal_all_Salmon_resSig_w_outlier.RDS")))
...
sidecols<- c("grey", "dodgerblue")[rlogdds$Diet]
...
DAVID_results_DESeq<- addList(david, row.names(resSig_Salmon), idType = "ENSEMBL_GENE_ID" , listName = sprintf("FF_vs_Mal_all_Sig_genes_DESeq2") ,listType = "Gene" )
...
saveRDS(cts,file = file.path(output_dir,sprintf("FF_vs_Mall_all_groups_counts_TPM_Sal_with_outliers.RDS")))
eset <- cts
...
if(length(terms)>2){
print(GOCluster(circ, dataGOplot$process, clust.by = "term", lfc.col = c('darkgoldenrod1','black','cyan1')))}}else{
  print("No significant term found!")
}
...
saveRDS(All_genes_entrez_rank, file = file.path(output_dir,sprintf("FF_vs_Mal_all_All_entrez_rank_Sal_w_outliers.RDS")))
...
sprintf("Barplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections");
print(barplot(x, showCategory = 30));
sprintf("Dotplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections");
print(dotplot(x, showCategory=15));
sprintf("Enrichment map plot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections");
...
if(!is.null(x)){
if(min(x@result$p.adjust)<0.05){
sprintf("Barplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections up regulated genes");
print(barplot(x, showCategory = 30));
sprintf("Dotplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections up regulated genes");
print(dotplot(x, showCategory=15));
sprintf("Enrichment map plot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections upregulated genes");
print(emapplot(x, showCategory = 15, color = "p.adjust",layout = "kk",colorEdge = TRUE));
...
if(!is.null(x)){
if(min(x@result$p.adjust)<0.05){
sprintf("Barplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections downregulated genes");
print(barplot(x, showCategory = 30));
sprintf("Dotplot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections downregulated genes");
print(dotplot(x, showCategory = 15));
sprintf("Enrichment map plot for the most enriched reactome pathway for Salmon/DESeq analysis for comparison FF vs Mal all sections downregulated genes")
...
EdgeR_s$samples$group<-selectMetadata$Diet
...
saveRDS(Top_Genes_Table, file = file.path(output_dir,sprintf("FF_vs_Mall_all_EdgeR_sig_Sal_w_outliers.RDS")))
```
...
DAVID_results_EdgeR<- addList(david, row.names(Topgenes_sfactor_edgeR_0.1_1$table), idType = "ENSEMBL_GENE_ID" , listName = sprintf("FF_vs_Mal_all_Sig_genes_EdgeR") ,listType = "Gene" )
...
if(dim(DAVID_results_chart_EdgeR_BP)>0){
...
saveRDS(Salmon_intersect,file = file.path(output_dir,sprintf("FF_vs_Mal_all_Salmon_intersect_w_outliers.RDS")))
```
